...
class CurvedDrawer extends StatefulWidget {
  ...
  CurvedDrawer({
    ...
    this.isEndDrawer = false
      ...
      assert(0 <= width && width <= 100),super(key: key);

  @override
  _CurvedDrawerState createState() => _CurvedDrawerState();
}

class _CurvedDrawerState extends State<CurvedDrawer> with SingleTickerProviderStateMixin {
  double _startingPos;
  int _endingIndex = 0;
  double _pos;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
       ...
       child: Stack(
         ...
         alignment: widget.isEndDrawer ? Alignment.centerRight :Alignment.centerLeft,
         children: <Widget>[
           Positioned(
            left: widget.isEndDrawer ? null : -widget.width * 0.6 - (75.0 - widget.width),
            right: widget.isEndDrawer ? -widget.width * 0.6 - (75.0 - widget.width) : null,
            top: _pos * size.height,
            height: size.height / _length,
            child: Center(
              child: Transform.translate(
                offset: Offset(
                  _isEnd * (1 - _buttonHide) * (75 + (widget.width * 0.2)), 0,
                ),
                child: _icon
              )
            ),
           ...
           Positioned(
             ...
             width:widget.width,
             child: CustomPaint(
               painter: DrawerCustomPainter(_pos, _length, widget.color, Directionality.of(context), widget.width, widget.isEndDrawer),
               child: Container(
                 width: widget.width,
                 height: size.height
               )
